...
object Surfaceatoms {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (docked, (closestRef, rmsd), score, molAsurfaceatoms, molBsurfaceatoms) = doMainDock(dockArgs)
  ...
  def doMainDock(dockArgs: DockArgs) = {
    ...
    num = 0
    for (atomB <- molA.surfaceAtoms){
      num = num + 1;
    }

    val molAsurfaceatoms = num
    (docked, getRMSD(docked, dockArgs), score, molAsurfaceatoms, molBsurfaceatoms)
  }
